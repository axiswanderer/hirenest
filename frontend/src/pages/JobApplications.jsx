...
import { Mail, Phone, Download, User, MessageSquare, Send, X, AlertCircle } from 'lucide-react';
...
const JobApplications = () => {
    ...
    const handleStatusChange = async (appId, newStatus) => {
        ...
        try {
            ...
            setStatusError(detail || 'Failed to update status.');
        }
    };
    ...
    const downloadResume = async (filename) => {
        if (!filename) return;
        try {
            const response = await api.get(`/files/resume/${filename}`, { responseType: 'blob' });
            ...
            setStatusError('Failed to download resume.');
        }
    };

    const getAvatarUrl = (avatarFilename) => {
        if (!avatarFilename) return null;
        const filename = avatarFilename.split(/[\\/]/).pop();
        return `${API_URL}/static/avatars/${filename}`;
    };
                            ...
                            <div className="flex-grow">
                                ...
                                <div className="mt-6 pt-4 border-t border-slate-100 flex gap-3">
                                    <button
                                        ...
                                        className="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-slate-800 disabled:opacity-50"
                                    >
